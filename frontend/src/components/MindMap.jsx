...
export default function MindMap({ data }) {
  ...
  return (
    ...
    // Links with smooth curves and animations
    const links = g.append('g')
      .attr('fill', 'none')
      .attr('stroke-linecap', 'round')
      .selectAll('path')
    ...
    // Botones +/− más grandes y visibles para expandir/colapsar
    const expandableNodes = node.filter(d => d.depth === 1 && (d.children || d.data._children))
    ...
    // Fondo blanco detrás de los textos para mejor legibilidad
    node.each(function(d) {
      const textNode = d3.select(this)
      const name = d.data.name || ''
      const maxLen = d.depth === 0 ? 40 : d.depth === 1 ? 35 : 30
      const displayText = name.length > maxLen ? name.slice(0, maxLen - 1) + '…' : name
      
      // Calcular ancho aproximado del texto
      const charWidth = d.depth === 0 ? 8.5 : d.depth === 1 ? 7.5 : 6.5
      const textWidth = displayText.length * charWidth
      const padding = 8
      
      // Posición del texto
      const hasChildren = d.children || d.data._children
      const xPos = hasChildren ? -12 : 12
      const xAnchor = hasChildren ? 'end' : 'start'
      
      // Fondo blanco
      textNode.append('rect')
        .attr('x', xAnchor === 'end' ? xPos - textWidth - padding : xPos)
        .attr('y', -12)
        .attr('width', textWidth + padding * 2)
        .attr('height', 24)
        .attr('rx', 4)
