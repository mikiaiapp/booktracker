...
# --- FASE 1: FICHA Y AUTOR (Inicia cadena siempre) ---
...
# --- FASE 2: ESTRUCTURA Y RESÚMENES (Inicia cadena siempre) ---
...
def process_book_phase2(user_id: str, book_id: str):
    ...
    async def _p2():
        async for db in get_user_db(user_id):
            ...
            process_book_phase3.delay(user_id, book_id, chain=True) # Inicia cadena
    ...
    async def _p3():
        async for db in get_user_db(user_id):
            ...
            for i, c in enumerate(char_list):
                update_progress(user_id, book_id, "phase3", int(60+(i/len(char_list)*20)), f"Analizando: {c['name']}")
                detail = await analyze_single_character(c["name"], c.get("is_main"), all_summaries, book.title)
                if detail:
...
def process_book_phase4(user_id: str, book_id: str, chain: bool = False):
    async def _p4():
        async for db in get_user_db(user_id):
            book = (await db.execute(select(Book).where(Book.id == book_id))).scalar_one_or_none()
            all_summaries = await _get_summaries_text(db, book_id)
            update_progress(user_id, book_id, "phase4", 85, "Redactando resumen global...")
...
def process_book_phase5(user_id: str, book_id: str, chain: bool = False):
    async def _p5():
        async for db in get_user_db(user_id):
            ...
            if chain:
                process_book_phase6.delay(user_id, book_id) # La 6 es el final, no necesita chain
            else:
                on_done(user_id, book_id)
...
def process_book_phase6(user_id: str, book_id: str):
    async def _p6():
        async for db in get_user_db(user_id):
            ...
            update_progress(user_id, book_id, "phase6", 95, "Grabando podcast...")
...
# --- BOTONES DE REANALIZAR (Sin encadenar) ---
